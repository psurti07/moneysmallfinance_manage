...
{
    public function companyLeads(Request $request){
        if($request->ajax()){
        ...
        return view('companyleads::index');
    }

    public function info(Request $request){
        $userDetails = UserRegistration::find($request->input('infoId'));
        $sourceDetails = DB::table('source_entry')->where('user_id', $userDetails->id)->orderByDesc('id')->get();
        ...
        return view('companyleads::modals.infodetails')->with($rec);
    }
    ...
    public function convertCustomer(Request $request){
        ...
        try{
            ...
            DB::transaction(function () use ($netamount, $cgstamount, $sgstamount, $igstamount, $request, $paymentid, $grandtotal, $cardno, $regdate, $regdatetime, $userDetails) {
                DB::insert('INSERT INTO membership_orders (rec_date, userid, registration_date, expiry_date, card_number, amount, paymentid) VALUES (?,?,?,?,?,?,?)',
                ...
                DB::update('UPDATE user_registrations SET rec_date = ?, update_date = ?, password = ?, refcode = ?, process_step = ?, isUser = ? WHERE id = ?',
                    [$regdatetime, $regdatetime, $passwordkey, $refcode, 5, 2, $userDetails->id]);
                $invoiceNo = getSiteKeyValue('newinvoiceno');
                $invfor = 1;
                $invprefix = "SA_";
                DB::insert('INSERT INTO invoices (rec_date, userid, cardid, inv_prefix, inv_number, inv_date, inv_price, inv_cgst, inv_sgst, inv_igst, inv_grandtotal) VALUES (?,?,?,?,?,?,?,?,?,?,?)',
